...
function OrderPanel({ userId, market, ticker, wallet, marketOpen, onComplete, position }) {
  ...
  const total = orderSide === 'BUY' ? grossAmount + fee : grossAmount - fee
  const risk = useMemo(() => {
    const validRisk = orderSide === 'BUY' && Number.isFinite(entry) && Number.isFinite(sl) && sl > 0 && sl < entry
    const validReward = orderSide === 'BUY' && Number.isFinite(tp) && tp > entry
  ...
  }, [entry, numericQuantity, orderSide, sl, tp])
  ...
  const validate = () => {
    ...
    if (orderSide === 'SELL' && (!position || position.status !== 'open')) return 'An open position is required for a reduce-only sell.'
    if (orderSide === 'SELL' && numericQuantity > position.quantity) return 'Sell quantity exceeds your open position.'
  ...
  const confirmOrder = async () => {
    ...
    try {
      if (orderType === 'market') {
        ...
        await createLimitOrder({ userId, symbol: market.symbol, side: orderSide, quantity: numericQuantity, limitPrice: entry, stopLoss: sl, takeProfit: tp })
        onComplete?.(`${orderSide === 'SELL' ? 'Reduce-only sell' : 'Buy'} limit placed at ${formatPrice(entry, market)}.`)
      }
  ...
  const summary = [
    ['Market', market.displaySymbol], ['Side', orderSide], ['Order type', orderType === 'market' ? 'Market' : 'Limit'],
    ['Reduce only', orderSide === 'SELL' ? 'Yes' : 'No'],
    ...
    ['Estimated fee', formatCurrency(fee)], [orderSide === 'BUY' ? 'Estimated total cost' : 'Estimated proceeds', formatCurrency(total)],
    ['Stop loss', orderSide === 'SELL' || sl === null ? 'Not set' : formatPrice(sl, market)], ['Take profit', orderSide === 'SELL' || tp === null ? 'Not set' : formatPrice(tp, market)],
  ...
  return <div>
    ...
    <form className="space-y-5 p-5" onSubmit={prepareOrder}>
      ...
      {orderSide === 'BUY' && <><div className="grid grid-cols-2 gap-3"><Input inputMode="decimal" label="Stop loss (optional)" min="0" onChange={(event) => { setStopLoss(event.target.value); setError('') }} placeholder="Below entry" step="any" type="number" value={stopLoss} /><Input inputMode="decimal" label="Take profit (optional)" min="0" onChange={(event) => { setTakeProfit(event.target.value); setError('') }} placeholder="Above entry" step="any" type="number" value={takeProfit} /></div><section className="space-y-3 rounded-lg border border-border bg-surface p-3" aria-labelledby="position-size-heading"><div className="flex items-center gap-2"><Calculator className="size-4 text-accent" /><div><h3 className="text-xs font-semibold" id="position-size-heading">Account Risk</h3><p className="text-[10px] text-muted">Simulation sizing tool</p></div></div><Input disabled={!risk.perUnit} inputMode="decimal" label="Risk amount (USD)" min="0" onChange={(event) => setRiskAmount(event.target.value)} placeholder={risk.perUnit ? 'Amount to risk' : 'Add a valid stop loss first'} step="any" type="number" value={riskAmount} /><div className="grid grid-cols-4 gap-2">{balanceRisks.map((percent) => <button className="h-8 rounded-md border border-border text-[11px] font-semibold text-muted hover:border-accent/50 hover:text-foreground disabled:opacity-40" disabled={!risk.perUnit} key={percent} onClick={() => setBalanceRisk(percent)} type="button">{percent}%</button>)}</div><div className="flex items-center justify-between text-xs"><span className="text-muted">Suggested quantity</span><span className="financial-value font-semibold">{suggestedQuantity > 0 ? suggestedQuantity.toFixed(MAX_DECIMAL_QUANTITY) : '—'}</span></div><Button disabled={!suggestedQuantity} fullWidth onClick={() => setQuantity(suggestedQuantity.toFixed(MAX_DECIMAL_QUANTITY))} size="sm" variant="secondary">Use suggested size</Button></section></>}
