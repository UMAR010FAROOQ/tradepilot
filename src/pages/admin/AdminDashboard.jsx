...
import { BanknoteArrowDown, BanknoteArrowUp, CandlestickChart, CircleDollarSign, BriefcaseBusiness, Users, Wallet } from 'lucide-react'
import { Link } from 'react-router-dom'
import Badge from '../../components/common/Badge.jsx'
import Card from '../../components/common/Card.jsx'
import PageHeader from '../../components/common/PageHeader.jsx'
import { AdminEmpty, AdminError, AdminLoading } from '../../components/admin/AdminState.jsx'
import { getPendingDeposits, getPendingWithdrawals, getPositions, getTrades, getUsers } from '../../services/adminService.js'
...
function AdminDashboard() {
  ...
  useEffect(() => {
    let active = true
    Promise.all([getUsers(), getPendingDeposits(), getPendingWithdrawals(), getTrades(), getPositions()])
      .then(([users, deposits, withdrawals, trades, positions]) => active && setData({ users, deposits, withdrawals, trades, positions }))
  ...
  const tradingVolume = data?.trades.reduce((sum, trade) => sum + (trade.grossAmount || 0), 0) || 0
  const tradingFees = data?.trades.reduce((sum, trade) => sum + (trade.fee || 0), 0) || 0
  const openPositions = data?.positions.filter((position) => position.status === 'open' && position.quantity > 0).length || 0
  const cards = data ? [
    ...
    { label: 'Total trades', value: data.trades.length, icon: CandlestickChart, to: '/admin/trades' },
    { label: 'Open positions', value: openPositions, icon: BriefcaseBusiness, to: '/admin/trades' },
    { label: 'Trading volume', value: formatCurrency(tradingVolume), icon: CircleDollarSign, to: '/admin/trades' },
    { label: 'Trading fees', value: formatCurrency(tradingFees), icon: CircleDollarSign, to: '/admin/trades' },
  ] : []
  ...
  return (
    <div className="space-y-6">
      ...
      {!data && !error ? <Card padding="none"><AdminLoading /></Card> : data && <>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(({ label, value, icon: Icon, to }) => <Link key={label} to={to}><Card className="h-full transition hover:border-accent/40" elevated><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium text-muted">{label}</p><p className="financial-value mt-3 truncate text-2xl font-semibold">{value}</p></div><span className="grid size-10 shrink-0 place-items-center rounded-lg bg-accent/10 text-accent"><Icon className="size-5" /></span></div></Card></Link>)}</div>
