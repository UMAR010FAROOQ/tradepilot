...
function Analytics() {
  const { currentUser } = useAuth(); const { wallet, loading: walletLoading } = useWallet()
  const [trades, setTrades] = useState([]); const [positions, setPositions] = useState([]); const [tickers, setTickers] = useState(new Map()); const [loading, setLoading] = useState(true); const [positionsLoading, setPositionsLoading] = useState(true); const [error, setError] = useState('')
  ...
  useEffect(() => subscribeToPositions(currentUser.uid, (items) => { setPositions(items); setPositionsLoading(false) }, (requestError) => { setError(getFirestoreErrorMessage(requestError)); setPositionsLoading(false) }), [currentUser.uid])
  ...
  const accountLoading = loading || positionsLoading || walletLoading
  const mainMetrics = [
